...
const weekReport = (path:string, na:string, initialDate:Date, pll:Array<ProjectItem> ):Promise<void>=>{
    ...
    return workbook.xlsx.readFile("./template.xlsx").then( ()=>{
        ...
        workbook.xlsx.readFile("./template.xlsx").then( ()=>{ //path
            ...
            sheet = workbook.addWorksheet(shhetTitle);
            sheet.model = Object.assign(template.model, {
                // @ts-ignore
            ...
            startDateCell.value = "自　"+initialDate.getFullYear()+"年　"+ format(initialDate.getMonth() + 1 )+"月　"+ format( initialDate.getDate() ) + "日";
            startDateCell.style.font={size: 13};
            endDateCell.value = "至　"+endDate.getFullYear()+"年　"+ format(endDate.getMonth() + 1 ) +"月　"+ format( endDate.getDate() ) + "日";          
            ...
            pll.forEach((item, index, arr)=>{
                ...
                if(first){
                    ...
                    projectRow = sheet.insertRow(insertI,  [ item.bango,item.na,"","","","","","","","", "", "" ], "o" );
                }
                ...
                item.sakugyoNaiyouList?.forEach((sakugyouItem) => {
                    ...
                    var goukei = sakugyouItem.shuu.reduce((accumulator, currentValue) => accumulator + currentValue)
                    pjGoukei += goukei;
                ...
                projectRow.values = [ item.bango,item.na,"","","","","","","","", "", pjGoukei ]
            ...
            for(let i = 4; i <= 11; i++){
                ...
                if(goukeiCell){
                    ...
                    if(i < 11 && zangyouCell){
                        zangyouCell.value ={
                            formula: "IF(" + letter + insertI + "=0,0," + letter + insertI + "-IF(OR(" + letter + "$7=2,"+letter + "$7=3),4,IF(" + letter + "$7=1,8,0)))", 
                            result: "", 
                            date1904: false
                        };
                    }
