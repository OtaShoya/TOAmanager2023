...
import UserSelect from "@/components/atmos/Select";
      ...
      if (memberForm) {
        ...
        if (memberInputs.length > 0) {
          memberInputs.forEach((el, index) => {
            ...
            if (nameAttribute && nameAttribute.includes("form1")) {
              let inputElement: any = el;
              styleString +=
                "#form-member option[value='" + inputElement.value + "']";
...
const AddPage = ({ socket, members, projectList }: any) => {
  ...
  const onSubmit: SubmitHandler<FormType> = (data) => {
    let p: Project = {
      ...
      cousuu: data.costs1,
      keihi: data.expenses,
      honkadouYoteiHi: data.scheduledDate
        ? data.scheduledDate.toDateString()
        : new Date().toDateString(),
      kashibi: data.startDate
        ? data.startDate.toDateString()
        : new Date().toDateString(),
      shuuryoubi: data.endDate
      ...
      <div className="flex flex-col space-y-5">
        ...
        <div className="flex">
          ...
          <div className="flex flex-col space-y-5 w-3/4 mr-3">
            ...
            <label className="edit-label flex flex-col">
              親プロジェクト
              <select className="edit-form" {...register("projectPar")}>
                <option value="0"></option>
          ...
          <div className="w-1/4">
            ...
            <div
              className="w-auto p-2 flex flex-col gap-y-2 border"
              id="form-member"
            >
              ...
              {form1.map((form, index) => (
                <div className="flex " key={form.id}>
                  ...
                  {/* <UserSelect {...register(`form1.${index}.projectMember`)} /> */}
                  <button
                    onClick={() => {
                      form1Remove(index);
                      refreshShain();
            ...
            <div className="w-auto p-2 flex flex-col gap-y-2 border">
              <table className="border-spacing-0 border-collapse">
                <thead className="block">
                  <tr>
                    <th className="border text-white column-back_color">
                      タスク
                    </th>
                    <th className="border text-white column-back_color">
                      作業内容
                ...
                <tbody className="block overflow-y-scroll h-32">
                  {form2.map((form, index) => (
                    <tr key={form.id} className="text-black">
                      <td className="p-0">
                        <select
                      ...
                      <td>
                        <input
                          ...
                          className="min-[1940px]:h-14 h-10"
                          onChange={(e) => {
